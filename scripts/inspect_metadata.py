...
def _strip_shared_affixes(values: List[str]) -> List[str]:
    ...
    return [v if v else orig for v, orig in zip(values, values)]
...
# TODO: add extra column filters if the accession has multiple sub-experiments.
subset_filters:
  default:
    "Assay Type": "{assay}"
...
featurecounts:
  ...
  feature_type: "exon"
  attribute: "gene_id"  # TODO: "gene_id" for Ensembl IDs or "gene_name" for symbols
  gtf_filter: ""
  option_sets:
    default:
      label: "Default"
...
# -- Selection policy (optional, exam-focused) --------------------------------
...
"""
    ...
    if "attribute" not in " ".join(todos).lower():
        todos.append("Check featureCounts attribute (gene_id vs gene_name)")
    ...
    comparison_names = []
    for t in treatments:
        comparison_names.append(f"{t}_vs_{ref_level}")
